...
fn sum_numbers_h(root: Option<&Rc<RefCell<TreeNode>>>, current_sum: i32) -> i32 {
    if let Some(r) = root {
        ...
        sum_numbers_h(b.left.as_ref(), current_sum) + sum_numbers_h(b.right.as_ref(), current_sum)
    } else {
        0
    }
}
...
mod tests {
    ...
    fn test_1() {
        assert_eq!(
            sum_numbers(to_bt(&[1.into(), 2.into(), 3.into()]).as_ref()),
            25
        );
    }
    ...
    fn test_2() {
        assert_eq!(
            sum_numbers(to_bt(&[4.into(), 9.into(), 0.into(), 5.into(), 1.into()]).as_ref()),
            1026
        );
    }
}
